...
<table width="100%" cellspacing="0" cellpadding="0">
  <tr>
    <td>
      <form action="yoga-export.php" method="POST">
    ...
    <td style="text-align: center;">
      <form action="yoga-db.php" method="POST">
...
<?php
$result = mysql_query("SELECT * FROM yoga ORDER BY lastname ASC");
...
while ($row = mysql_fetch_array($result)) {
  ...
  $Message .= "<br>";
  if (!empty($row['numberstudents'])) $Message .= "Total number of Yoga and Pilates students: " . $row['numberstudents'] . "<br>";
  if (!empty($row['instructor'])) $Message .= "Active Yoga and/or Pilates instructor? " . $row['instructor'] . "<br>";
  if (!empty($row['hear'])) $Message .= "How did you hear about the Yoga Gift Packs? " . $row['hear'] . "<br>";
?>
  <tr<?php echo $bg; ?>>
