...
switch ($_REQUEST['a']) {
	case "allrec":
		$result = mysql_query("SELECT firstname,lastname,businessname,address,address2,city,state,zip,email,phone,numberstudents,hear FROM womensfitness ORDER BY lastname ASC");
    $headings = array("First","Last","Women's Fitness Club Name","Address","Address 2","City","State","Zip Code","E-Mail","Phone","Number of Female Members","How did you hear about the Women's Fitness Sample Packs?");
		...
		break;
	case "unexprec":
		$result = mysql_query("SELECT firstname,lastname,businessname,address,address2,city,state,zip,email,phone,numberstudents,hear FROM womensfitness WHERE exported = '' ORDER BY lastname ASC");
    $headings = array("First","Last","Women's Fitness Club Name","Address","Address 2","City","State","Zip Code","E-Mail","Phone","Number of Female Members","How did you hear about the Women's Fitness Sample Packs?");
		$lastcol = "N";
		break;
  case "allemail":
    $result = mysql_query("SELECT email FROM womensfitness ORDER BY email ASC");
    $headings = array("E-Mail");
		$lastcol = "A";
		break;
	case "unexpemail":
    $result = mysql_query("SELECT email FROM womensfitness WHERE exported = '' ORDER BY id ASC");
    $headings = array("E-Mail");
		$lastcol = "A";
		break;
}
...
mysql_query("UPDATE womensfitness SET exported = 'on'");
